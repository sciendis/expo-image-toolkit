...
export const ZoomRange = function () {
    ...
    const onRangeLayout = useCallback((event) => {
        const { layout } = event.nativeEvent;
        if (rangeLayout.x !== layout.x ||
    ...
    const { moveRangeBar, animatedTextProps, styledRangeAnimated, currentZoom } = useMoveZoomRangeBar({ currentX, rangeLayout });
    return (<View style={styles.container} onLayout={onRangeLayout}>
      <GestureHandlerRootView>
        <GestureDetector gesture={moveRangeBar}>
          <Animated.View style={[styles.pointer, styledRangeAnimated]}>
            <AnimatedText animatedProps={animatedTextProps} style={styles.text} value={`${currentZoom.toFixed(2)}x`} editable={false}/>
          </Animated.View>
        </GestureDetector>
      </GestureHandlerRootView>
    </View>);
