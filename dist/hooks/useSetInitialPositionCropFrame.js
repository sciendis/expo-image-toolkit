...
export const useSetInitialPositionCropFrame = function () {
    const { image, imageLayout, initialBoxScale, initialBoxPosition, boxScale, boxPosition, containerLayout, } = useImageEditorContext();
    useEffect(() => {
        if (!image || imageLayout.width <= 0 || imageLayout.height <= 0)
            return;
        try {
            calculateImageOffset({ image, imageLayout }).then(({ displayedImageWidth, displayedImageHeight }) => {
                boxScale.set({
...
//# sourceMappingURL=useSetInitialPositionCropFrame.js.map